...
const FormattedText = ({ text, isDark, className = "", noBubbles = false }) => {
  if (!text || text === "N/A") return null;

  // Pre-process: if noBubbles is set, we treat backticks as emphasis instead of code
  const rawText = noBubbles ? text.replace(/`/g, '**') : text;
      ...
      {lines.map((line, idx) => {
        ...
        if (isBullet || isNumberBullet) {
          ...
          if (colonMatch && !bulletContent.includes('`')) {
            ...
            // If noBubbles is active, we skip the specialized command rendering and use standard flow
            if (looksLikeCommand && !noBubbles) {
              return (
...
const Section = ({ title, icon, content, color, isDark, isCode = false, onListen, isListening, isSynthesizing, noBubbles = false }) => {
  ...
  // Improved conditional logic for Light/Dark mode compatibility
  const containerClasses = isCode 
    ? (isDark 
  ...
  return (
    <div className="mb-4 sm:mb-6 last:mb-0 relative group">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          {onListen && (
            <button
              ...
              className={`w-7 h-7 sm:w-6 sm:h-6 flex items-center justify-center rounded-full transition-all active:scale-90 ${isListening
      ...
      <div className={`p-3 sm:p-4 rounded-xl border transition-all duration-300 font-mono text-xs sm:text-sm overflow-x-auto ${containerClasses}`}>
        {isCode ? <div className="whitespace-pre leading-relaxed overflow-x-auto pb-1">{processedContent}</div> : <FormattedText text={processedContent} isDark={isDark} className="text-xs sm:text-sm" noBubbles={noBubbles} />}
      </div>
    </div>
  );
};
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <div className="w-full sm:w-auto flex flex-1 justify-end gap-1.5 sm:gap-2">
          <button
            ...
            className={`px-3 py-1.5 rounded-xl border text-[9px] sm:text-[10px] font-bold uppercase flex items-center gap-2 transition-all active:scale-95 ${activeSpeechId === 'summary' ? 'bg-blue-600 border-blue-500 text-white animate-pulse' :
      ...
      {data.isTechnicalQuestion && data.generalAnswer ? (
        <div className="flex flex-col gap-4 animate-fadeIn">
          <div className={`p-5 sm:p-7 rounded-2xl border transition-all duration-300 shadow-sm leading-relaxed ${isDark ? 'bg-slate-900 border-slate-800 text-slate-300' : 'bg-white border-slate-200 text-slate-700'}`}>
            ...
            <FormattedText text={data.generalAnswer} isDark={isDark} className="text-sm sm:text-base" noBubbles={true} />
          </div>
